...
function basicTrain(ns: NS, category = ns.args[1]) {
  const { sleeve } = ns;
...
/** @param {NS} ns */
export function buySleeveAugs(ns: NS) {
  const { sleeve } = ns;
  [...Array(sleeve.getNumSleeves()).keys()].forEach((i) => {
